...
<div class="row" v-if="true" id="${uuid}">
            ...
            <div class="form-inline clearfix">
                <div class="form-group pull-left">
                    <div class="search-header-title">
                        <i class="fa fa-tachometer"></i> 项目监控
                    </div>
                </div>
                ...
                <div class="form-group pull-right" style="width: 180px;margin-right: 8px;">
                    <div class="input-group input-group-sm">
                        ...
                        <input class="form-control" v-model="platformName" placeholder="按项目名称搜索">
                    </div>
                </div>
            </div>
        </div>
    ...
    <div class="col-md-12">
        <table  class="table table-striped table-bordered table-hover">
            ...
            <tbody>
                <template v-for="platform in filterItems" :key="platform.platformId">
                    <template v-if="platform.statMonitors && platform.statMonitors.length > 0">
                        <tr  v-for="(statMonitor, index) in platform.statMonitors">
                            <td :rowspan="platform.typeCount" v-if="index == 0">
                                <a :module_id="'module_monitor_platform_' + platform.platformId" :href="'monitor/platform_host/list?projectid=' + platform.platformId + '&monitorTime=' + monitorTime" data-html="true" class="btn btn-xs ajaxify">{{platform.platformName}}</a>
                                <a class="hidden"><i class="fa fa-tachometer"></i> {{platform.platformName}}-监控</a></td>
                            </td>
...
<script>
	require(["jquery", "../custom/scripts/monitor/platform_list_init", "bootstrap_datepicker"], function($, page) {
        //Metronic.init();
        page.init({
            uuid: '${uuid}'
        });
