		...
		<div class="portlet box blue">
			...
			<div class="portlet-body">
				<div class="table-toolbar">
					<@security.authorize ifAnyGranted="ROLE_CONFIGURATION_BASIS_CMDBDEVICEBRAND_ADD">
						<div class="btn-group">
							<a class="btn yellow ajaxify" module_id="module_cmdb_device_brand_add" href="configuration/cmdb_device_brand/add?displayName=设备品牌">
							<i class="fa fa-plus-square-o"></i> 添加设备品牌
							</a>
						</div>
					</@security.authorize>
				</div>
					...
					<table class="table table-striped table-bordered table-hover" id="${uuid}">
						<thead>
							...
							<tr role="row" class="filter">
								<td>
									<input type="text" class="form-control form-filter input-sm" name="brandname">
								</td>
...
<#--END PAGE JAVASCRIPT-->
<script>
	require(["jquery", "../custom/scripts/cmdb/configuration/cmdbDeviceBrand/cmdbDeviceBrand_list_init"], function($, page) {
		$(function() {
			Metronic.init();
			page.init({
				uuid: '${uuid}',
				hasEdit: '<@security.authorize ifAnyGranted="ROLE_CONFIGURATION_BASIS_CMDBDEVICEBRAND_EDIT">true</@security.authorize>'
			});
		});
	});
</script>