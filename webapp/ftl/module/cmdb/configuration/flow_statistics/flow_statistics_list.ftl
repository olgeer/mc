				...
				<template id="${uuid}_tpl">
					<tbody>
							<tr v-for="item in items">
								<td>
									<a module_id="module_flow_activity_list_14953" 
									:href="'configuration/flow_activity/list?processName=' + item.process_name" data-html="true" 
									class="btn btn-xs ajaxify">{{item.process_name}}</a>
									<a class="hidden"><i class="fa fa-cog"></i> {{item.process_name}}-步骤分析</a>
								</td>
...
<script>
	require(["jquery", "../custom/scripts/cmdb/configuration/flow_statistics/flow_statistics_list_init"], function($, page) {
		$(function() {
			Metronic.init();
			page.init({
				uuid: '${uuid}',
