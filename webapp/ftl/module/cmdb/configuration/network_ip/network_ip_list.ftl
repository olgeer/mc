				...
				<div class="table-toolbar">
					...
					<div class="btn-group">
						<a class="btn yellow generate">
							<i class="fa fa-plus-square-o"></i> 生成IP
						</a>
					</div>
				</div>
					...
					<table class="table table-striped table-bordered table-hover" id="${uuid}">
						<thead>
							...
							<tr role="row" class="filter">
								<td>
									<input type="hidden" class="form-control form-filter input-sm" name="belongs_ip_pool_id" value="${belongs_ip_pool_id!}">
								...
								<td>
									<@dictSelect name="status" type="cmdb_network_ip_status" class="form-control form-filter input-sm" />
								</td>
...
<script>
	require(["jquery", "../custom/scripts/cmdb/configuration/network_ip/network_ip_list_init"], function($, page) {
		$(function() {
			Metronic.init();
			page.init({
				uuid: '${uuid}',
				poolId: '${belongs_ip_pool_id!}',
				hasEdit: 'true'
			});
		});
	});
