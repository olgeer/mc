	...
	<div class="col-md-12">
		...
		<div class="portlet box blue">
			<div class="portlet-title">
				<div class="caption">
					<i class="fa fa-archive"></i>应用类型列表
				</div>
			</div>
			<div class="portlet-body">
				<div class="table-toolbar">
					<@security.authorize ifAnyGranted="ROLE_CONFIGURATION_APPLICATION_APPTYPE_EDIT">
						<div class="btn-group">
							<a class="btn yellow ajaxify" module_id="module_cmdbAppType_add" href="configuration/cmdbAppType/add?displayName=应用类型">
							<i class="fa fa-plus-square-o"></i> 添加应用类型
							</a>
						</div>
					</@security.authorize>
				</div>
					...
					<table class="table table-striped table-bordered table-hover" id="${uuid}">
						<thead>
							...
							<tr role="row" class="filter">
								<td>
									<input type="text" class="form-control form-filter input-sm" name="cmdbAppType">
								</td>
								<td>
									<input type="text" class="form-control form-filter input-sm" name="app_detail_type">
								</td>
...
<#--END PAGE JAVASCRIPT-->
<script>
	require(["jquery", "../custom/scripts/cmdb/configuration/cmdbAppType/cmdbAppType_list_init"], function($, page) {
		$(function() {
			Metronic.init();
			page.init({
				uuid: '${uuid}',
				hasEdit: '<@security.authorize ifAnyGranted="ROLE_CONFIGURATION_APPLICATION_APPTYPE_EDIT">true</@security.authorize>'
			});
		});
	});
</script>