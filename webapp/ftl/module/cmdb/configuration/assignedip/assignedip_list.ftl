	...
	<div class="col-md-12">
		...
		<div class="portlet box blue">
			<div class="portlet-title">
				<div class="caption">
					<i class="fa fa-magic"></i>IP列表
				</div>
			</div>
			<div class="portlet-body">
				<div class="table-toolbar">
					<@security.authorize ifAnyGranted="ROLE_CONFIGURATION_NETWORK_ASSIGNEDIP_EDIT">
						<div class="btn-group">
							<a class="btn yellow ajaxify" module_id="module_assignedip_add" href="configuration/assigned_ip/add?displayName=IP">
							<i class="fa fa-plus-square-o"></i> 添加IP
							</a>
						</div>
					</@security.authorize>
				</div>
					...
					<table class="table table-striped table-bordered table-hover" id="${uuid}">
						<thead>
							...
							<tr role="row" class="filter">
								...
								<td>
									<input type="text" class="form-control form-filter input-sm" name="ai.vlan">
								</td>
								<td>
...
<#--END PAGE JAVASCRIPT-->
<script>
	require(["jquery", "../custom/scripts/cmdb/configuration/assignedip/assignedip_list_init"], function($, page) {
		$(function() {
			Metronic.init();
			page.init({
				uuid: '${uuid}',
				hasEdit: '<@security.authorize ifAnyGranted="ROLE_CONFIGURATION_NETWORK_ASSIGNEDIP_EDIT">true</@security.authorize>'
			});
		});
	});
</script>